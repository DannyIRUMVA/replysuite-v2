...
const selectBestPlanSlug = (memberships: any[] = []) =>
  memberships
    .filter((membership) => isMembershipCurrentlyUsable(membership))
    .map((membership) =>
      String((membership?.plans as any)?.internal_slug || "").toLowerCase(),
    )
...
const loadActiveMemberships = async (
  ...
  if (fallback.error) {
    console.warn(
      "[Auth Middleware] Membership lookup failed:",
...
export default defineNuxtRouteMiddleware(async (to, from) => {
  ...
  if (
    !user.value &&
    (to.path.startsWith("/dashboard") || to.path === "/verify")
  ) {
    ...
    if (!sessionData?.session?.user) {
      ...
      return navigateTo({
        path: "/login",
        query:
          !isLoggingOut.value && to.fullPath && to.fullPath !== "/dashboard"
  ...
  const previewOnboarding =
    ...
    ["1", "true", "yes"].includes(
      String(to.query.onboarding || "").toLowerCase(),
    );
  ...
  if (
    ...
    if (!hasMembership && auth.userId.value) {
      const supabase = useSupabaseClient();
      const { data, error } = await loadActiveMemberships(
        supabase,
        auth.userId.value,
      );
      hasMembership = error
        ? true
        : data.some((membership: any) =>
  ...
  if (
    ...
    if (
      to.path.startsWith("/dashboard/integrations/instagram") &&
      !isGoldOrEnterprise
    ) {
      return navigateTo("/dashboard/pricing");
    }
    ...
    if (to.path.startsWith("/dashboard/orders")) {
      return navigateTo(
        isEnterprise ? "/dashboard/appointments" : "/dashboard/pricing",
      );
    }
    ...
    if (
      to.path.startsWith("/dashboard/agents/tools/website-builder") &&
      !isEnterprise
    ) {
      return navigateTo("/dashboard/pricing");
    }
  ...
  if (user.value && to.path.startsWith("/dashboard") && auth.userId.value) {
    ...
    try {
      ...
      const rowsByCode = onboardingRows.reduce(
      ...
      const hasCompanyProfile = Boolean(
        String(profileRes.data?.company_name || "").trim() &&
        String(profileRes.data?.contact_email || "").trim(),
      );
      ...
      const supportedStepCodes = stepsRes.data
        ?.map((step: any) => step.code)
        .filter(Boolean) || ["verify_account", "create_chatbot"];
      const needsOnboarding = supportedStepCodes.some(
        (code) => rowsByCode[code] ?? !inferredCompletion[code],
      );

      forceOnboarding.value =
        previewOnboarding || (!verified && needsOnboarding);
      onboardingChecked.value = true;
      ...
      console.error(
        "[Auth Middleware] Failed to evaluate onboarding state:",
        error,
      );
      forceOnboarding.value = false;
      onboardingChecked.value = false;
