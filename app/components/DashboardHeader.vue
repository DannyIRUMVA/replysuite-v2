...
import { Bot, ChevronDown, HelpCircle, MessageSquare, Plug, Zap, TrendingUp } from 'lucide-vue-next'
...
const { data: headerStats } = useAsyncData('dashboard-header-status-pills', async () => {
  ...
  const [sessionsRes, whatsappRes, instagramRes] = await Promise.all([
    ...
    supabase.from('instagram_accounts').select('id', { count: 'exact', head: true }).eq('user_id', userId.value),
  ])
...
const dashboardPills = computed(() => [
  { label: 'Assistants ready', value: headerStats.value?.assistants || 0, icon: Bot },
  { label: 'Customer chats', value: headerStats.value?.conversations || 0, icon: MessageSquare },
  { label: 'Channels connected', value: headerStats.value?.channels || 0, icon: Plug },
])
...
onBeforeUnmount(() => document.removeEventListener('click', closeStatusPopover))
watch(() => route.fullPath, () => { isStatusPopoverOpen.value = false })
...
<template>
  <header class="flex flex-col md:flex-row md:items-center justify-between gap-1.5 mb-0">
    <!-- Brand (Mobile Only) -->
    <div class="flex md:hidden items-center justify-between w-full mb-1">
      <div class="flex items-center gap-3">
        <div class="h-[1.625rem] w-[1.625rem] bg-gradient-to-tr from-primary to-primary-accent rounded-lg flex items-center justify-center shadow-lg shadow-primary/20">
      ...
      <div class="flex items-center gap-2">
        ...
        <NuxtLink to="/dashboard/pricing" class="dashboard-header-action-btn !p-2.5" aria-label="Upgrade plan">
    ...
    <div class="min-h-[2rem] flex-1 animate-in fade-in slide-in-from-left-4 duration-500">
      ...
      <template v-else-if="showPageContext">
        <h1 class="text-base md:text-lg font-black text-foreground leading-tight">
          {{ pageContext.title }}
        </h1>
        <p class="text-foreground/50 text-[10px] md:text-[11px] font-medium mt-0.5">{{ pageContext.subtitle }}</p>
      </template>
      <div v-else ref="statusPopoverRef" class="relative hidden md:block">
        ...
        >
          ...
          <span><strong class="font-black tabular-nums text-foreground">{{ assistantCount }}</strong> assistants</span>
          <ChevronDown :class="['h-3.5 w-3.5 text-foreground/35 transition-transform', isStatusPopoverOpen ? 'rotate-180' : '']" />
        </button>
            ...
            <div class="space-y-1">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-medium text-foreground/60">{{ pill.label }}</span>
                </div>
                <span class="text-sm font-black tabular-nums text-foreground">{{ pill.value }}</span>
              </div>
            </div>
          </div>
        </Transition>
    ...
    <div class="hidden items-center justify-end gap-2.5 border-t border-foreground/5 pt-1.5 sm:flex md:border-t-0 md:pt-0">
      <button
        type="button"
