export const usePlanAccess = () => {
  ...
  const normalizedPlanSlug = computed(() =>
    String(planSlug.value || "").toLowerCase(),
  );
  ...
  const isEnterprisePlan = computed(() =>
    ["enterprise-ready", "enterprise"].includes(normalizedPlanSlug.value),
  );
  const isSilverOrHigher = computed(
    () => isSilverPlan.value || isGoldPlan.value || isEnterprisePlan.value,
  );
  const isGoldOrHigher = computed(
    () => isGoldPlan.value || isEnterprisePlan.value,
  );
  ...
  return {
    ...
    canUseSchoolRegistration: isEnterprisePlan,
  };
};
