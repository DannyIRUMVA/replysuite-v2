...
const raskGraphInputs = [
  {
    label: 'Business knowledge',
    detail: 'Rask website, FAQs, service notes, and approved answers',
    icon: BrainCircuit
  },
  {
    label: 'Customer channels',
    detail: 'Website chat, WhatsApp, Instagram comments, and DMs',
    icon: MessageSquareMore
  },
  {
    label: 'Booking context',
    detail: 'Availability, appointment rules, calendar-backed reservations',
    icon: CalendarCheck
  }
]

const raskGraphOutputs = [
  {
    label: 'Instant answers',
    detail: 'Customers get Rask-specific replies instead of generic guesses',
    icon: MessageCircle
  },
  {
    label: 'Qualified follow-up',
    detail: 'The assistant captures needs, contact details, and next steps',
    icon: ScanSearch
  },
  {
    label: 'Human handoff',
    detail: 'Your team takes over when a conversation needs judgment',
    icon: ShieldCheck
  }
]

const raskExampleMessages = [
  { role: 'customer', text: 'Hi Rask, can you help me choose the right AI assistant for customer support?' },
  { role: 'assistant', text: 'Yes. I can explain ReplySuite, answer from Rask-approved content, and help you compare website chat, WhatsApp, Instagram, and booking workflows.' },
  { role: 'customer', text: 'Can I start with website chat and add WhatsApp later?' },
  { role: 'assistant', text: 'Yes. Start with a trained website assistant, then connect WhatsApp and Instagram features as your plan grows.' }
]
...
<template>
  <div class="relative overflow-x-hidden">
    ...
    <section class="max-w-7xl mx-auto px-6 py-14 md:py-18 border-t border-foreground/5">
      <div class="grid lg:grid-cols-[1.05fr_0.95fr] gap-8 lg:gap-10 items-stretch">
        <div class="glass-card border-foreground/10 bg-foreground/[0.02] p-5 sm:p-7 lg:p-8 overflow-hidden">
          <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
            <div class="max-w-2xl">
              <span class="badge-gradient mb-4">Workflow graph</span>
              <h2 class="text-3xl md:text-5xl font-extrabold leading-tight text-foreground">
                How ReplySuite works, using Rask Assistant as an example.
              </h2>
            </div>
            <p class="text-sm text-foreground/60 font-semibold leading-relaxed max-w-sm">
              One trained assistant sits between your business knowledge, customer channels, and team handoff rules.
            </p>
          </div>

          <div class="relative grid gap-5 lg:grid-cols-[1fr_auto_1fr] items-center">
            <div class="space-y-3">
              <div v-for="item in raskGraphInputs" :key="item.label"
                class="rounded-2xl border border-foreground/10 bg-background-card p-4 shadow-sm">
                <div class="flex items-start gap-3">
                  <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">
                    <component :is="item.icon" class="h-4 w-4" />
                  </div>
                  <div>
                    <p class="text-sm font-black text-foreground">{{ item.label }}</p>
                    <p class="mt-1 text-xs font-semibold leading-relaxed text-foreground/55">{{ item.detail }}</p>
                  </div>
                </div>
              </div>
            </div>

            <div class="relative flex items-center justify-center py-2 lg:px-2">
              <div class="hidden lg:block absolute left-[-70px] right-[-70px] top-1/2 h-px bg-gradient-to-r from-primary/0 via-primary/45 to-primary/0"></div>
              <div class="hidden lg:block absolute left-[-28px] top-[38%] h-px w-14 rotate-12 bg-primary/25"></div>
              <div class="hidden lg:block absolute right-[-28px] top-[62%] h-px w-14 -rotate-12 bg-primary/25"></div>
              <div class="relative z-10 w-full max-w-[260px] rounded-[28px] border border-primary/20 bg-background-card p-5 text-center shadow-2xl shadow-primary/10">
                <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                  <Bot class="h-7 w-7" />
                </div>
                <p class="text-[10px] font-black uppercase tracking-[0.22em] text-primary">Center node</p>
                <h3 class="mt-2 text-xl font-black text-foreground">Rask Assistant</h3>
                <p class="mt-2 text-xs font-semibold leading-relaxed text-foreground/55">
                  Trained once, then used across supported channels with business rules and safe handoff.
                </p>
              </div>
            </div>

            <div class="space-y-3">
              <div v-for="item in raskGraphOutputs" :key="item.label"
                class="rounded-2xl border border-foreground/10 bg-background-card p-4 shadow-sm">
                <div class="flex items-start gap-3">
        ...
        <div class="rounded-[28px] border border-foreground/10 bg-background-card p-4 sm:p-5 shadow-2xl shadow-foreground/5">
          <div class="flex items-center justify-between gap-3 border-b border-foreground/8 pb-4">
            <div class="flex items-center gap-3 min-w-0">
              <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                <Bot class="h-5 w-5" />
              </div>
              <div class="min-w-0">
                <p class="truncate text-sm font-black text-foreground">Rask Assistant preview</p>
                <p class="text-[11px] font-semibold text-foreground/55">Example customer conversation</p>
              </div>
            </div>
            <span class="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-emerald-500">Live path</span>
          </div>

          <div class="mt-5 space-y-3">
            <div v-for="message in raskExampleMessages" :key="message.text" class="flex"
              :class="message.role === 'customer' ? 'justify-end' : 'justify-start'">
              <div class="max-w-[88%] rounded-2xl px-4 py-3 text-xs font-semibold leading-relaxed shadow-sm"
                :class="message.role === 'customer'
                  ? 'rounded-br-md bg-sky-500 text-white shadow-sky-500/15'
                  : 'rounded-bl-md border border-foreground/8 bg-background text-foreground/70'">
                <p class="mb-1 text-[9px] font-black uppercase tracking-[0.18em] opacity-70">
                  {{ message.role === 'customer' ? 'Customer' : 'Rask Assistant' }}
                </p>
                {{ message.text }}
              </div>
            </div>
          ...
          <div class="mt-5 rounded-2xl border border-primary/15 bg-primary/5 p-4">
            <p class="text-xs font-black uppercase tracking-[0.18em] text-primary">What the graph means</p>
            <p class="mt-2 text-sm font-semibold leading-relaxed text-foreground/65">
              Rask trains the assistant on approved business knowledge. ReplySuite then uses that context to answer customers on connected channels and guide conversations toward support, sales, or bookings.
            </p>
          </div>
        </div>
      </div>
