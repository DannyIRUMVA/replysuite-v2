...
<template>
  <main class="tool-detail-compact min-h-screen overflow-hidden pb-12">
    <section class="relative mx-auto max-w-[72rem] px-4 py-10 sm:px-6 md:py-12">
      <div class="absolute right-0 top-0 -z-10 h-56 w-[58%] rounded-full bg-primary/[0.08] blur-[110px]"></div>
      <div class="grid gap-6 lg:grid-cols-[0.88fr_1.12fr] lg:items-start">
        <div class="lg:sticky lg:top-28">
          ...
          <div class="mt-5 inline-flex items-center gap-2 rounded-[0.7rem] border border-primary/20 bg-primary/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-primary">
            <Sparkles class="h-3.5 w-3.5" /> {{ currentTool.badge }}
          </div>
          <h1 class="mt-4 text-3xl font-extrabold leading-[1.02] tracking-[-0.045em] text-foreground md:text-4xl lg:text-[3.15rem]">
            {{ currentTool.title }}
          </h1>
          <p class="mt-4 text-sm font-semibold leading-6 text-foreground/60 md:text-[0.95rem]">
            {{ currentTool.description }} Fill the fields or start with sample values, then copy the finished response.
          </p>

          <div class="mt-5 rounded-[1rem] border border-primary/15 bg-primary/[0.08] p-5 shadow-sm shadow-black/5">
            <p class="text-[10px] font-black uppercase tracking-[0.14em] text-primary">Need this every day?</p>
            <p class="mt-2 text-sm font-medium leading-6 text-foreground/65">
              Train ReplySuite once and let your assistant reply across website chat, WhatsApp, Instagram, and booking conversations.
            </p>
            <NuxtLink :to="`/register?ref=${currentTool.slug}`" class="btn-gradient mt-4 inline-flex items-center gap-2 rounded-[0.78rem] px-5 py-3 text-sm font-bold">
        ...
        <form class="rounded-[1rem] border border-foreground/10 bg-background-card/45 p-4 shadow-sm shadow-black/5 backdrop-blur-xl sm:p-5 lg:p-6" @submit.prevent="generateReply">
          <div class="grid gap-4 sm:grid-cols-2">
            <label class="block">
              <span class="text-xs font-black uppercase tracking-[0.14em] text-foreground/55">Business type</span>
              <select v-model="businessType" class="mt-2 w-full rounded-[0.72rem] border border-foreground/10 bg-background px-3 py-2.5 text-sm font-semibold text-foreground outline-none transition focus:border-primary/50">
            ...
            <label class="block">
              ...
              <input v-model="businessName" type="text" placeholder="Optional" class="mt-2 w-full rounded-[0.72rem] border border-foreground/10 bg-background px-3 py-2.5 text-sm font-semibold text-foreground outline-none transition placeholder:text-foreground/45 focus:border-primary/50" />
            </label>
          </div>
          ...
          <label class="mt-5 block">
            <span class="flex items-center justify-between gap-3 text-xs font-black uppercase tracking-[0.14em] text-foreground/55">
              ...
              <span class="rounded-[0.6rem] bg-foreground/[0.04] px-2.5 py-1 text-[10px] tracking-normal text-foreground/55">Required</span>
            </span>
            <textarea v-model="input" :maxlength="inputMaxLength" :placeholder="currentTool.inputPlaceholder" rows="7" class="mt-2 w-full resize-y rounded-[0.82rem] border border-foreground/10 bg-background px-4 py-3 text-sm leading-6 text-foreground outline-none transition placeholder:text-foreground/45 focus:border-primary/50 focus:ring-4 focus:ring-primary/10"></textarea>
          </label>
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <button type="button" class="rounded-[0.7rem] border border-primary/30 bg-primary/10 px-3 py-2 text-xs font-black text-primary transition hover:border-primary/50" @click="fillSampleValues">
              Fill sample values
            </button>
            <button v-for="example in currentTool.examples" :key="example.label" type="button" class="rounded-[0.7rem] border border-foreground/10 px-3 py-2 text-xs font-bold text-foreground/65 transition hover:border-primary/30 hover:text-primary" @click="fillExample(example.input)">
              Use {{ example.label }}
            </button>
          </div>
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <div class="mt-2 grid grid-cols-2 gap-2">
                <button v-for="option in currentTool.tones" :key="option.value" type="button" :aria-pressed="tone === option.value" class="rounded-[0.7rem] border px-3 py-2 text-sm font-bold transition focus:outline-none focus:ring-4 focus:ring-primary/10" :class="tone === option.value ? 'border-primary bg-primary/10 text-primary' : 'border-foreground/10 text-foreground/60 hover:border-foreground/20 hover:text-foreground'" @click="tone = option.value">
                  {{ option.label }}
                </button>
              </div>
            </div>
            <div>
              ...
              <div class="mt-2 grid grid-cols-3 gap-2">
                <button v-for="option in currentTool.lengths" :key="option.value" type="button" :aria-pressed="length === option.value" class="rounded-[0.7rem] border px-3 py-2 text-sm font-bold transition focus:outline-none focus:ring-4 focus:ring-primary/10" :class="length === option.value ? 'border-primary bg-primary/10 text-primary' : 'border-foreground/10 text-foreground/60 hover:border-foreground/20 hover:text-foreground'" @click="length = option.value">
                  {{ option.label }}
                </button>
              </div>
            </div>
          ...
          <label class="mt-5 block">
            ...
            <input v-model="extraContext" type="text" placeholder="Example: Ask them to reply with a booking time." class="mt-2 w-full rounded-[0.72rem] border border-foreground/10 bg-background px-3 py-2.5 text-sm font-semibold text-foreground outline-none transition placeholder:text-foreground/45 focus:border-primary/50 focus:ring-4 focus:ring-primary/10" />
          </label>

          <div class="mt-6 grid gap-3 sm:grid-cols-[1fr_auto]">
            <button type="submit" class="btn-gradient inline-flex w-full items-center justify-center gap-2 rounded-[0.78rem] px-5 py-3 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-60" :disabled="!canGenerate">
            ...
            <button type="button" class="inline-flex items-center justify-center gap-2 rounded-[0.78rem] border border-foreground/10 px-5 py-3 text-sm font-bold text-foreground/60 transition hover:border-foreground/20 hover:text-foreground" @click="clearForm">
              <Eraser class="h-4 w-4" /> Clear
            </button>
          </div>

          <p v-if="errorMessage" role="alert" class="mt-4 rounded-[0.82rem] border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-300">
            {{ errorMessage }}
          </p>

          <section v-if="reply" aria-live="polite" class="mt-5 rounded-[1rem] border border-primary/15 bg-primary/5 p-4">
            <div class="mb-3 flex items-center justify-between gap-3">
              ...
              <button type="button" class="inline-flex items-center gap-2 rounded-[0.7rem] border border-foreground/10 px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-foreground/65 transition hover:border-primary/30 hover:text-primary" @click="copyReply">
                <Check v-if="copied" class="h-4 w-4" />
                <Clipboard v-else class="h-4 w-4" />
                {{ copied ? 'Copied' : 'Copy' }}
              </button>
            </div>
            <p class="whitespace-pre-wrap text-sm leading-6 text-foreground/80">{{ reply }}</p>
    ...
    <section class="mx-auto max-w-[72rem] px-4 sm:px-6">
      <div class="rounded-[1rem] border border-foreground/10 bg-background-card/45 p-5 text-center shadow-sm shadow-black/5 sm:p-6">
        <p class="text-sm font-semibold leading-6 text-foreground/65">
          Want replies that already know your policies, booking rules, tone, and channels?
        </p>
        <NuxtLink :to="`/register?ref=${currentTool.slug}`" class="mt-4 inline-flex items-center justify-center gap-2 rounded-[0.78rem] border border-primary/30 bg-primary/10 px-5 py-3 text-sm font-bold text-primary transition hover:border-primary/50 hover:bg-primary/15">
