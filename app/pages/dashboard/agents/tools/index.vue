...
import { Bot, CalendarCheck2, CalendarDays, ChevronDown, CreditCard, Globe2, Loader2, Save, Search } from 'lucide-vue-next'
...
const toolStats = computed(() => {
  ...
  const usedCount = rows.filter((row) => ['Connected', 'Ready'].includes(row.status)).length
  const actionCount = rows.filter((row) => ['attention', 'pending'].includes(row.tone)).length
...
<template>
  <div class="w-full min-w-0 space-y-4 pb-20 lg:pb-12">
    <section v-if="isLoading" class="rounded-2xl border border-foreground/10 bg-background-card shadow-sm">
      <div class="flex flex-col gap-2 border-b border-foreground/10 p-3 lg:flex-row lg:items-center">
      ...
      <div class="grid gap-2 border-b border-foreground/10 p-3 sm:grid-cols-2 xl:grid-cols-4">
        <div v-for="card in 4" :key="`stat-skeleton-${card}`" class="rounded-xl border border-foreground/10 bg-foreground/[0.02] p-3">
      ...
      <div class="overflow-hidden">
        <div class="hidden md:block">
          <div class="grid grid-cols-[2fr_1fr_1fr_1.5fr_1fr] gap-4 border-b border-foreground/10 px-4 py-3">
            <Skeleton v-for="column in 5" :key="`head-${column}`" height="0.75rem" radius="999px" />
          </div>
          <div v-for="row in 6" :key="`tool-skeleton-${row}`" class="grid grid-cols-[2fr_1fr_1fr_1.5fr_1fr] gap-4 border-b border-foreground/10 px-4 py-3 last:border-b-0">
        ...
        <div class="grid gap-2 p-3 md:hidden">
          <div v-for="row in 4" :key="`mobile-tool-skeleton-${row}`" class="rounded-xl border border-foreground/10 bg-foreground/[0.02] p-3">
    ...
    <div v-else>
      <main class="min-w-0">
        <section class="min-w-0 rounded-2xl border border-foreground/10 bg-background-card shadow-sm">
          <div class="flex flex-col gap-2 border-b border-foreground/10 p-3 xl:flex-row xl:items-center">
            ...
            <div class="grid min-w-0 flex-1 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-[minmax(14rem,1fr)_auto_auto_auto] xl:justify-end">
              ...
              <select v-model="toolStatusFilter" class="h-10 w-full rounded-xl border border-foreground/10 bg-background px-3 text-xs font-black uppercase tracking-widest text-foreground/60 outline-none focus:border-primary/40 lg:w-36">
                <option v-for="status in toolStatuses" :key="status" :value="status">{{ status === 'all' ? 'All status' : status }}</option>
              </select>
              <select v-model="toolCategoryFilter" class="h-10 w-full rounded-xl border border-foreground/10 bg-background px-3 text-xs font-black uppercase tracking-widest text-foreground/60 outline-none focus:border-primary/40 lg:w-44">
                <option v-for="category in toolCategories" :key="category" :value="category">{{ category === 'all' ? 'All categories' : category }}</option>
              </select>
              <button @click="saveTools" :disabled="!hasChanges || isSaving" class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-[10px] font-black uppercase tracking-widest text-black transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 lg:w-auto">
          ...
          <div class="grid gap-2 border-b border-foreground/10 p-3 sm:grid-cols-2 xl:grid-cols-4">
            <article v-for="stat in toolStats" :key="stat.label" :class="['rounded-xl border p-3 transition', statCardClass(stat.tone)]">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <p class="truncate text-[10px] font-black uppercase tracking-[0.18em] text-foreground/40">{{ stat.label }}</p>
                  <p class="mt-1 text-2xl font-black leading-none text-foreground">{{ stat.value }}</p>
                  <p class="mt-2 truncate text-[11px] font-bold text-foreground/45">{{ stat.detail }}</p>
                </div>
                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-background text-foreground/55 ring-1 ring-foreground/10">
          ...
          <div class="hidden overflow-x-auto md:block">
            <table class="w-full min-w-[760px] text-left">
              ...
              <tbody class="divide-y divide-foreground/10">
                <tr v-for="row in filteredToolRows" :key="row.id" class="group transition hover:bg-primary/[0.03]">
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-3">
                      <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-foreground/5 text-foreground/50"><component :is="row.icon" class="h-4 w-4" /></div>
                      <div class="min-w-0">
                ...
                <tr v-if="filteredToolRows.length === 0">
                  <td colspan="5" class="px-4 py-8 text-center text-xs font-bold text-foreground/45">No tools match the current filters.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="grid gap-2 p-3 md:hidden">
            <article v-for="row in filteredToolRows" :key="`mobile-${row.id}`" class="rounded-xl border border-foreground/10 bg-foreground/[0.02] p-3">
              <div class="flex items-start gap-3">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-foreground/5 text-foreground/55">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="truncate text-sm font-black text-foreground">{{ row.name }}</p>
                      <p class="mt-1 text-[10px] font-black uppercase tracking-widest text-foreground/35">{{ row.category }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-foreground/10 bg-background px-2 py-1 text-[10px] font-black uppercase tracking-widest text-foreground/60">
                      <span :class="['h-1.5 w-1.5 rounded-full shadow-md', statusDotClass(row.tone)]" />
                      {{ row.status }}
                    </span>
                  </div>
                  <p class="mt-3 line-clamp-2 text-xs font-semibold leading-relaxed text-foreground/50">{{ row.connectedData }}</p>
                  <button type="button" class="mt-3 inline-flex h-9 w-full items-center justify-center rounded-lg border border-foreground/10 bg-foreground/5 px-3 text-[10px] font-black uppercase tracking-widest text-foreground/65 transition hover:border-primary/30 hover:text-primary" @click="handleToolAction(row.id)">{{ row.action }}</button>
                </div>
              </div>
            </article>
            <div v-if="filteredToolRows.length === 0" class="rounded-xl border border-dashed border-foreground/10 p-6 text-center text-xs font-bold text-foreground/45">No tools match the current filters.</div>
          </div>
        </section>
