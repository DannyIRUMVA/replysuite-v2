...
const isPremium = computed(() =>
  ["silver", "gold", "enterprise-ready"].includes(planSlug.value || ""),
);
...
const activeTab = ref<
  "identity" | "design" | "conversation" | "security" | "tools" | "skills"
>("identity");
...
useHead({
  title: computed(() =>
    agent.value?.name
      ? `${agent.value.name} | Assistant Settings`
      : "Assistant Settings | ReplySuite",
  ),
});
...
const selectedLanguageCodes = computed({
  ...
  set: (codes: string[]) => {
    ...
    const fallbackCode = getChatbotLanguageCode(
      languageSettings.value.fallbackLanguage,
    );
...
const fallbackLanguageOptions = computed(() =>
  languageCodeOptions
...
const primaryLanguageCode = computed(() =>
  getChatbotLanguageCode(form.value.default_language),
);
const fallbackLanguageCode = computed(() =>
  getChatbotLanguageCode(languageSettings.value.fallbackLanguage),
);
...
const toggleSupportedLanguage = (code: string) => {
  ...
  if (current.has(code)) {
    if (
      code === primaryLanguageCode.value ||
      code === fallbackLanguageCode.value
    )
      return;
    current.delete(code);
  } else {
...
const websiteConnectionIsActive = computed(
  () => form.value.is_public && form.value.allowed_domains.length > 0,
);
const canDisableLocalhostTesting = computed(
  () => websiteConnectionIsActive.value,
);
...
const addAllowedDomain = () => {
  ...
  if (normalized === "localhost" || normalized === "127.0.0.1") {
    notify.warn(
      "Use localhost testing instead of adding localhost as an approved production domain.",
    );
    return;
  }
  ...
  if (form.value.allowed_domains.length >= maxWebsiteDomains.value) {
    notify.error(
      `Your current plan allows up to ${maxWebsiteDomains.value} website domain${maxWebsiteDomains.value === 1 ? "" : "s"} per assistant.`,
    );
    return;
  }
...
const setFallbackLanguageDefaults = () => {
  ...
  selectedLanguageCodes.value = Array.from(
    new Set([primaryCode, ...selectedLanguageCodes.value, fallbackCode, "en"]),
  );
};
...
const fetchChatbotLanguages = async () => {
  try {
    ...
    const fallback =
      rows.find((row: any) => row.is_fallback) ||
      rows.find((row: any) => row.language_code === "en") ||
      primary;

    form.value.default_language = getChatbotLanguageName(primary.language_code);
    languageSettings.value = {
      supportedCodes: Array.from(
        new Set(rows.map((row: any) => row.language_code).filter(Boolean)),
      ),
    ...
    console.warn(
      "[Languages] chatbot_languages unavailable, using legacy default_language only:",
      err,
    );
...
const saveChatbotLanguages = async () => {
  ...
  const codes = Array.from(
    new Set([primaryCode, ...selectedLanguageCodes.value, fallbackCode]),
  );
  ...
  try {
    ...
    console.warn(
      "[Languages] Could not persist chatbot language mappings yet:",
      err,
    );
    languageSettings.value.tableAvailable = false;
  }
  ...
  try {
    ...
    if (data) {
      ...
      form.value = {
        ...
        enabled_tools: Array.isArray(data.enabled_tools)
          ? data.enabled_tools.filter((tool: string) => tool !== "orders")
          : [],
        tools_config: {
          ...(data.tools_config || {}),
  ...
  try {
    const { error } = await supabase
      ...
      .update({
        ...
        enabled_tools: form.value.enabled_tools.filter(
          (tool: string) => tool !== "orders",
        ),
    ...
    notify.success(
      languageSettings.value.tableAvailable
...
<template>
  <div class="mx-auto max-w-7xl space-y-5 pb-24 pt-3">
      ...
      >
        <div class="min-w-0">
          ...
          <h1 class="dashboard-section-title mt-2 truncate">
            {{ form.name || agent?.name || "Assistant" }}
          </h1>
          <p class="mt-2 max-w-2xl text-sm leading-relaxed text-foreground/65">
            Configure identity, conversation behavior, website design, tools,
            skills, and domain security from one compact workspace.
          </p>
        </div>
    ...
    <!-- Tab Switcher -->
    <div
      class="flex w-full gap-2 overflow-x-auto rounded-[0.39rem] border border-foreground/10 bg-background-card/45 p-1.5 shadow-sm shadow-black/5 lg:w-fit"
    >
      <button
        v-for="tab in [
    ...
    >
      <div class="lg:col-span-8 space-y-5">
        <!-- Identity -->
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6"
        >
          <h3 class="text-xs font-bold text-foreground/60 mb-4">Identity</h3>
          <div class="space-y-6">
            <div>
              <label class="block text-xs font-semibold text-foreground/50 mb-3"
            ...
            <div>
              ...
              />
              <div
                class="mt-4 p-4 rounded-[0.39rem] bg-primary/5 border border-primary/10 flex gap-3"
              >
                <Info class="w-4 h-4 text-primary shrink-0" />
                <p class="text-xs text-foreground/50 leading-relaxed">
                  This protocol overrides default behaviors. Define exactly how
                  you want the AI to represent your brand.
                </p>
              </div>
            </div>
          </div>
        ...
        <!-- Danger Zone -->
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6 border-red-500/10 bg-red-500/[0.01]"
        >
          <h3 class="text-xs font-bold text-red-500/70 mb-4">Danger zone</h3>
          <div
            class="flex items-center justify-between p-5 rounded-[0.39rem] bg-foreground/[0.01] border border-foreground/5"
          >
            <div>
              <p class="text-sm font-bold text-foreground mb-1">
                Delete assistant
              </p>
              <p class="text-xs text-foreground/50">
                Permanently remove this assistant from your workspace.
              </p>
            </div>
            <button
              @click="handleDelete"
      ...
      <div class="lg:col-span-4 space-y-6">
        <!-- Visibility Toggle -->
        <div
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5"
        >
          <div class="flex items-center justify-between mb-4">
            <h4 class="text-xs font-semibold text-foreground/50">Deployment</h4>
            <component
              :is="form.is_public ? Globe : Lock"
          ...
          <button
            ...
            :class="[
              ...
              form.is_public
                ...
                : 'bg-foreground/5 border-foreground/5 text-foreground/50',
            ]"
          >
            <div>
              <p class="text-xs font-semibold mb-1">
                {{ form.is_public ? "Public mode active" : "Internal only" }}
              </p>
              <p class="text-xs opacity-60">
                {{
                  form.is_public
                    ? "Website embed access enabled."
                    : "Keep this assistant private."
                }}
              </p>
            </div>
            <div
              :class="[
                ...
                form.is_public ? 'bg-primary' : 'bg-foreground/10',
              ]"
            >
              <div
                :class="[
          ...
          <p class="mt-4 text-xs text-foreground/50 leading-relaxed">
            Public mode allows your assistant to be embedded on approved
            websites.
          </p>
        </div>
        ...
        <!-- Languages -->
        <div
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 relative z-10"
        >
          <div class="flex items-center justify-between mb-4">
            <div>
              <h4 class="text-xs font-semibold text-foreground/50">
                Languages
              </h4>
              <p class="mt-1 text-xs text-foreground/40">
                Primary, supported and fallback languages
              </p>
            </div>
          ...
          <div class="space-y-5">
            <div>
              <label class="block text-xs font-semibold text-foreground/50 mb-2"
                ...
                >
                  <span>
                    ...
                    <span class="block text-xs opacity-60"
                      >{{ language.nativeName }} · {{ language.focus }}</span
                    >
                  </span>
                  <span
                    :class="[
          ...
          <p class="text-xs text-foreground/50 leading-relaxed mt-5">
            The runtime detects each visitor message and replies in a supported
            language. Keep English as fallback for safest recovery.
          </p>
          <p
            v-if="!languageSettings.tableAvailable"
            class="mt-3 rounded-[0.39rem] border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-300 leading-relaxed"
          >
            Legacy mode active until the language database migration is
            deployed. The assistant still uses runtime language detection.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              :class="[
                'w-10 h-5 rounded-full relative transition-colors p-1',
                form.ai_disclosure ? 'bg-primary' : 'bg-foreground/10',
              ]"
            >
              <div
                :class="[
                  'w-3 h-3 bg-white rounded-full transition-all',
                  form.ai_disclosure ? 'translate-x-5' : 'translate-x-0',
                ]"
              />
            </div>
          </button>
          <p class="mt-4 text-xs text-foreground/50 leading-relaxed">
            In the EU and Brazil, AI-generated content must be identifiable.
            This toggle appends transparency metadata to all responses.
          </p>
        </div>
        ...
        >
          ...
          <div class="space-y-5">
            <div class="flex items-center gap-4">
              <div class="p-2 rounded-[0.39rem] bg-foreground/5">
                <MessageSquare class="w-4 h-4 text-foreground/50" />
              </div>
              <div>
                <p class="text-xs font-bold text-foreground">
                  {{ agent?.session_count || 0 }} Conversations
                </p>
            ...
            <div class="flex items-center gap-4">
              <div class="p-2 rounded-[0.39rem] bg-foreground/5">
                <Shield class="w-4 h-4 text-foreground/50" />
              </div>
              <div>
    ...
    >
      ...
      <div class="lg:col-span-8 space-y-5">
        ...
        >
          <div class="flex items-center justify-between mb-4">
            <div>
              <h3 class="text-xs font-bold text-foreground/60 mb-1">
                Color Presets
              </h3>
              <p class="text-xs text-foreground/50">
                Quick-apply brand palettes
              </p>
            </div>
            <button
              @click="resetDesign"
              class="flex items-center gap-1.5 text-xs font-bold text-foreground/50 hover:text-foreground transition-colors"
            >
          ...
          <div class="grid grid-cols-4 sm:grid-cols-8 gap-3">
            <button
              ...
              :style="{
                background: `linear-gradient(135deg, ${preset.secondary} 0%, ${preset.primary}40 100%)`,
              }"
            >
              <div
                class="absolute bottom-1.5 right-1.5 w-4 h-4 rounded-full border border-foreground/20"
                :style="{ backgroundColor: preset.primary }"
              />
              <span
                class="absolute inset-0 flex items-center justify-center text-[8px] font-bold text-foreground/50 group-hover:text-foreground/50 transition-colors"
              >
                {{ preset.name }}
              </span>
            </button>
        ...
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6"
        >
          <h3 class="text-xs font-bold text-foreground/60 mb-4">
            Custom Colors
          </h3>
          <div class="grid grid-cols-2 gap-5">
            ...
            <div class="space-y-3">
              <label class="block text-xs font-semibold text-foreground/50">
                <Pipette
                  class="w-3 h-3 inline-block mr-1 text-primary"
                />Primary Color
              </label>
              <div
                class="flex items-center gap-3 p-3 bg-foreground/[0.02] border border-foreground/5 rounded-[0.39rem]"
              >
                <div class="relative">
                  <input
                    type="color"
                  ...
                  />
                  <div
                    class="w-7 h-7 rounded-[0.39rem] border-2 border-foreground/10 shadow-lg cursor-pointer transition-transform hover:scale-110"
                ...
                <div class="flex-1">
                  <input
                    ...
                    placeholder="#D4AF37"
                  />
                  <p class="text-[8px] text-foreground/50 mt-0.5">
            ...
            <div class="space-y-3">
              <label class="block text-xs font-semibold text-foreground/50">
                <Pipette
                  class="w-3 h-3 inline-block mr-1 text-foreground/50"
                />Background Color
              </label>
              <div
                class="flex items-center gap-3 p-3 bg-foreground/[0.02] border border-foreground/5 rounded-[0.39rem]"
        ...
        <!-- Style & Layout -->
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6 space-y-5"
        >
          <h3 class="text-xs font-semibold text-foreground/50">
            Style & Layout
          </h3>
            ...
            <div class="grid grid-cols-3 gap-3">
              ...
              >
                <div class="mb-2 h-5 flex items-center justify-center">
                  <div
                    class="h-4 w-16 bg-foreground/10"
                    :style="{
                      borderRadius:
                        style.value === 'pill'
                          ? '9999px'
          ...
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              >
                <!-- Mini screen mockup -->
                <div
                  class="w-16 h-10 bg-foreground/5 rounded-[0.39rem] relative border border-foreground/10"
                >
                  <div
        ...
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6 relative overflow-hidden"
          :class="{ 'opacity-50 pointer-events-none': !isPremium }"
        >
          <div
            v-if="!isPremium"
            class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/40 backdrop-blur-[2px]"
          >
            ...
            <p class="text-xs text-foreground/50 mt-1">
              Upgrade to Silver or Gold to unlock
            </p>
          </div>

          <h3
            class="text-xs font-bold text-foreground/60 mb-4 flex items-center justify-between"
          >
            Launcher Customization
            <span
              v-if="isPremium"
              class="bg-primary/20 text-primary px-2 py-0.5 rounded text-[8px]"
              >PREMIUM ACTIVE</span
            >
          </h3>
          ...
          <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-6">
              ...
              <div class="space-y-3">
                <label class="block text-xs font-semibold text-foreground/50">
                  <Pipette class="w-3 h-3 inline-block mr-1 text-primary" />Icon
                  Color
                </label>
                <div
                  class="flex items-center gap-3 p-3 bg-foreground/[0.02] border border-foreground/5 rounded-[0.39rem]"
                >
                  <div class="relative">
                    ...
                    />
                    <div
                      class="w-7 h-7 rounded-[0.39rem] border-2 border-foreground/10 shadow-lg cursor-pointer transition-transform hover:scale-110 flex items-center justify-center overflow-hidden"
                      :style="{
                        backgroundColor: form.launcher_icon_color || '#ffffff',
                      }"
                    >
                      <div
            ...
            <div class="space-y-3">
              <label class="block text-xs font-semibold text-foreground/50"
                >Widget Icon (Launcher & Chat)</label
              >
      ...
      <div class="lg:col-span-4 sticky top-5 sm:p-6">
        ...
        >
          <div class="flex items-center justify-between mb-5">
            ...
            <a
              :href="`/widget/${chatbotId}`"
              target="_blank"
              class="p-2 hover:bg-foreground/5 rounded-[0.39rem] transition-all text-foreground/50 hover:text-primary"
          ...
          >
            ...
            <div
              ...
              :style="{
                ...
                borderBottom: `1px solid ${form.primary_color}30`,
              }"
            >
              <div
            ...
            <div class="p-4 space-y-3 min-h-[180px]">
              ...
              <div class="flex gap-2 items-end">
                <div
                  class="w-6 h-6 rounded-full shrink-0 flex items-center justify-center text-[8px] font-bold"
                  :style="{
                    backgroundColor: form.primary_color,
                    color: form.secondary_color,
                  }"
                ...
                <div
                  class="max-w-[75%] px-3 py-2 text-xs text-foreground/80 leading-relaxed"
                  :style="{
              ...
              <div class="flex gap-2 items-end justify-end">
                <div
                  class="max-w-[75%] px-3 py-2 text-xs text-black font-bold leading-relaxed"
                  :style="{
                    backgroundColor: form.primary_color,
                    borderRadius: bubbleRadius,
                  }"
              ...
              <div class="flex gap-2 items-end">
                ...
                >
                  Of course! I'm {{ form.name || "your AI assistant" }}. What do
                  you need?
                </div>
              </div>
            </div>
            ...
            >
              <div
                class="flex-1 bg-foreground/5 rounded-[0.39rem] px-3 py-2 text-xs text-foreground/30"
              >
                Type a message...
              </div>
              <div
                class="w-7 h-7 rounded-[0.39rem] flex items-center justify-center shrink-0"
                :style="{ backgroundColor: form.primary_color }"
              >
                <svg
                  class="w-3 h-3"
                  :style="{ fill: form.secondary_color }"
                  viewBox="0 0 24 24"
                >
            ...
            <div class="text-center py-2 border-t border-white/5">
              <span class="text-[8px] text-foreground/30 font-bold"
                >Powered by ReplySuite</span
              >
            </div>
          </div>
          ...
          <!-- Position indicator -->
          <div
            class="mt-4 p-4 bg-foreground/[0.02] rounded-[0.39rem] border border-foreground/5 space-y-4"
          >
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-1.5">
                <div
                  ...
                  :style="{ backgroundColor: form.primary_color }"
                />
                <span class="text-xs text-foreground font-bold">{{
            ...
            <div class="flex items-center justify-between">
              <p class="text-xs text-foreground/50 font-bold">
                Launcher Preview
              </p>
              <div
                ...
                :style="{
                  ...
                  borderRadius:
                    form.launcher_style === 'circle'
                      ? '50%'
                      : form.launcher_style === 'square'
                        ? '0px'
                        : form.launcher_style === 'rounded-square'
                          ? '10px'
                          : '9999px',
                }"
              >
                <component
                  :is="
                    launcherIcons.find((i) => i.name === form.launcher_icon)
                      ?.icon || MessageSquare
                  "
                  class="w-5 h-5"
                  :style="{
                    color:
                      form.launcher_icon_color ||
                      ((parseInt(form.launcher_color.slice(1, 3), 16) * 299 +
                        parseInt(form.launcher_color.slice(3, 5), 16) * 587 +
                        parseInt(form.launcher_color.slice(5, 7), 16) * 114) /
                        1000 >
                      125
                        ? '#000'
                        : '#fff'),
    ...
    >
      <div class="lg:col-span-8 space-y-5">
        ...
        >
          ...
          <div class="grid md:grid-cols-3 gap-4">
            <div>
              ...
              >
              <select
                v-model="form.tools_config.conversation_settings.tone"
                class="w-full bg-foreground/5 border border-foreground/10 rounded-[0.39rem] px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50"
              >
            ...
            <div>
              ...
              >
              <select
                v-model="form.tools_config.conversation_settings.responseLength"
                class="w-full bg-foreground/5 border border-foreground/10 rounded-[0.39rem] px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50"
              >
            ...
            <div>
              ...
              <select
                v-model="
                  form.tools_config.conversation_settings.leadCaptureMode
                "
                class="w-full bg-foreground/5 border border-foreground/10 rounded-[0.39rem] px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50"
              >
        ...
        >
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <button
              @click="
                form.tools_config.conversation_settings.greetOncePerSession =
                  !form.tools_config.conversation_settings.greetOncePerSession
              "
              :class="[
                'p-5 rounded-[0.39rem] border text-left transition-all',
                form.tools_config.conversation_settings.greetOncePerSession
            ...
            >
              ...
              <p class="text-xs text-foreground/50">
                Avoid restarting the conversation after it has already begun.
              </p>
            </button>
            <button
              @click="
                form.tools_config.conversation_settings.avoidRepeatingQuestions =
                  !form.tools_config.conversation_settings
            ...
            >
              ...
              <p class="text-xs text-foreground/50">
                Do not ask for the same detail again when it is already known.
              </p>
            </button>
            <button
              @click="
                form.tools_config.conversation_settings.askOneQuestionAtATime =
                  !form.tools_config.conversation_settings.askOneQuestionAtATime
              "
              :class="[
                'p-5 rounded-[0.39rem] border text-left transition-all',
            ...
            >
              ...
              <p class="text-xs text-foreground/50">
                Keep follow-up simple instead of stacking multiple requests at
                once.
              </p>
            </button>
            <button
              @click="
                form.tools_config.conversation_settings.continueFromPreviousAnswer =
                  !form.tools_config.conversation_settings
                    .continueFromPreviousAnswer
              "
              :class="[
                'p-5 rounded-[0.39rem] border text-left transition-all',
                form.tools_config.conversation_settings
            ...
            >
              <p class="text-xs font-semibold text-foreground mb-1">
                Continue from prior answer
              </p>
              <p class="text-xs text-foreground/50">
                Treat short customer replies as answers to the last question and
                move forward.
              </p>
            </button>
          </div>
        ...
        >
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-foreground/50 mb-3"
                >Website Chat Style</label
              >
              <select
                v-model="
                  form.tools_config.conversation_settings.websiteReplyStyle
                "
                class="w-full bg-foreground/5 border border-foreground/10 rounded-[0.39rem] px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50"
              >
            ...
            <div>
              <label class="block text-xs font-semibold text-foreground/50 mb-3"
                >WhatsApp Reply Style</label
              >
              <select
                v-model="
                  form.tools_config.conversation_settings.whatsappReplyStyle
                "
                class="w-full bg-foreground/5 border border-foreground/10 rounded-[0.39rem] px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50"
              >
                <option value="short">Short</option>
      ...
      <div class="lg:col-span-4 space-y-6">
          ...
          <div class="space-y-4">
            ...
            >
              <div>
                ...
                <p class="text-xs opacity-60">
                  Remember what was already said inside the active conversation.
                </p>
              </div>
              <div
                :class="[
            ...
            <button
              @click="
                form.tools_config.conversation_settings.contactMemoryEnabled =
                  !form.tools_config.conversation_settings.contactMemoryEnabled
              "
              :class="[
                'w-full p-4 rounded-[0.39rem] border transition-all flex items-center justify-between text-left',
            ...
            >
              <div>
                <p class="text-xs font-semibold mb-1">
                  Returning Contact Memory
                </p>
                <p class="text-xs opacity-60">
                  Remember useful context for repeat contacts across sessions,
        ...
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 border-foreground/10 bg-foreground/[0.02]"
        >
          <h3 class="text-xs font-bold text-foreground/60 mb-4">
            How this helps
          </h3>
          <div
            class="space-y-3 text-xs text-foreground/50 leading-relaxed font-bold"
          >
    ...
    <div v-else-if="activeTab === 'tools'" class="space-y-10">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div class="lg:col-span-4 space-y-6">
          ...
          >
            <h3 class="text-xs font-bold text-foreground/60 mb-4">
              Intelligence Overlook
            </h3>
            <div class="space-y-5">
              <div
                class="p-5 rounded-[0.39rem] bg-foreground/[0.01] border border-foreground/5 flex items-center justify-between"
              >
                <div class="flex items-center gap-3">
                  <Zap class="w-5 h-5 text-foreground/50" />
                  <p class="text-xs font-semibold text-foreground/50">
                    Tools Enabled
                  </p>
                </div>
    ...
    >
      <div class="lg:col-span-8 space-y-5">
        <section
          class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6 text-center py-12"
        >
          <div
            class="w-14 h-14 bg-primary/10 rounded-[0.39rem] flex items-center justify-center mx-auto mb-4"
          >
            <Sparkles class="w-7 h-7 text-primary" />
          </div>
          <h3 class="text-xl font-bold text-foreground mb-4 italic-none">
            Skills
          </h3>
          <p
            class="text-xs text-foreground/50 max-w-md mx-auto leading-relaxed font-bold"
          >
            Skills help your assistant handle specialized tasks and stay aligned
            with your business use case.
          </p>
      ...
      <div class="lg:col-span-4 space-y-6">
        ...
        >
          ...
          <p class="text-xs text-foreground/40 leading-relaxed mb-4">
            Add custom knowledge sources for this assistant from the training
            area.
          </p>
          <NuxtLink
            :to="`/dashboard/agents/training?id=${chatbotId}`"
          ...
          >
            ...
            <span
              class="text-xs font-semibold text-foreground/60 group-hover:text-foreground"
              >Open training</span
            >
          </NuxtLink>
        </section>
    ...
    <div v-else-if="activeTab === 'security'" class="max-w-3xl space-y-5">
      ...
      >
        <div class="flex items-center gap-4 mb-4">
          <div
            class="p-3 rounded-[0.39rem] bg-primary/10 border border-primary/20"
          >
          ...
          <div>
            ...
            <p class="text-xs text-foreground/50 mt-1">
              Prevent unauthorized websites from embedding your widget
            </p>
          </div>
        </div>
        ...
        <div class="space-y-6">
          <div
            class="p-5 rounded-[0.39rem] bg-foreground/[0.01] border border-foreground/5 space-y-4"
          >
            <div class="flex items-start gap-4">
              ...
              <div class="space-y-2">
                ...
                <p class="text-xs text-foreground/50 leading-relaxed">
                  Once you add approved domains, all other origins are blocked.
                  Localhost stays available for testing until this connection is
                  active and you decide to disable it.
                </p>
                <p class="text-xs text-primary/80 leading-relaxed">
                  Current plan limit: {{ maxWebsiteDomains }} website domain{{
                    maxWebsiteDomains === 1 ? "" : "s"
                  }}
                  per assistant.
                </p>
              </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <div
                class="flex items-center justify-between gap-4 p-4 mt-4 rounded-[0.39rem] bg-foreground/[0.02] border border-foreground/10"
              >
                <div>
                  <p class="text-xs font-bold text-foreground">
                    Localhost Testing
                  ...
                  <p class="text-xs text-foreground/50 mt-1">
                    {{
                      canDisableLocalhostTesting
                        ? "Disable localhost after launch if you want production-only enforcement."
                        : "Localhost remains enabled until the website connection is active."
                    }}
                  </p>
                </div>
                <button
      ...
      <!-- Advanced Options -->
      <section
        class="rounded-[0.39rem] border border-foreground/10 bg-background-card/45 shadow-sm shadow-black/5 p-5 sm:p-6 opacity-50 grayscale pointer-events-none"
      >
        <h3
          class="text-xs font-semibold text-foreground/50 mb-4 flex items-center gap-2"
        >
          Advanced Security
          <span
            class="bg-primary/20 text-primary px-2 py-0.5 rounded text-[8px]"
        ...
        <div class="space-y-4">
          <div
            class="flex items-center justify-between p-4 rounded-[0.39rem] bg-foreground/[0.01] border border-foreground/5"
          >
            <div>
