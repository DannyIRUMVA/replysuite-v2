...
<template>
  <div class="h-[calc(100vh-6rem)] flex flex-col pt-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 shrink-0">
      <div class="max-w-xl">
        <h2 class="text-xl font-bold tracking-tight text-foreground mb-2 uppercase">Conversations Logs</h2>
        <p class="text-foreground/50 text-sm">Analyze raw engagement threads generated by your AI counterparts.</p>
      </div>
      ...
      <div class="flex items-center gap-3">
        <button 
          ...
          class="flex items-center gap-2 px-5 py-2.5 bg-foreground/5 border border-foreground/10 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-foreground hover:bg-foreground/10 hover:border-foreground/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
        >
    ...
    <!-- Interface Structure -->
    <div class="flex-1 min-h-0 bg-background border border-foreground/5 rounded-[2rem] overflow-hidden flex shadow-2xl">
        
        <!-- Sidebar Contacts / Sessions -->
        <div class="w-full md:w-[350px] border-r border-foreground/5 flex flex-col bg-foreground/[0.02]">
            <div class="p-5 border-b border-foreground/5 shrink-0">
                <div class="relative">
                    <Search class="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-foreground/50" />
                    <input type="text" placeholder="Search conversations..." class="w-full bg-foreground/5 border border-foreground/5 rounded-xl py-3 pl-10 pr-4 text-xs text-foreground uppercase tracking-widest focus:outline-none focus:border-primary/30 transition-all">
                </div>
            </div>

            <div class="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-2 custom-scrollbar">
                <div v-if="loadingSessions && sessions.length === 0" class="flex flex-col items-center justify-center h-40 text-foreground/50 space-y-4">
                    <div class="w-6 h-6 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                    <span class="text-[10px] uppercase tracking-widest font-bold">Decrypting Logs...</span>
                </div>
                
                <div v-else-if="sessions.length === 0" class="flex flex-col items-center justify-center p-8 text-center text-foreground/50 h-40">
                ...
                <div v-else v-for="session in sessions" :key="session.id" 
                     ...
                     <div class="flex items-center justify-between mb-3">
                         <span class="text-xs font-bold text-foreground uppercase tracking-widest flex items-center gap-2">
                         ...
                         <span class="text-[9px] text-foreground/50 tracking-widest whitespace-nowrap font-black">
                            <template v-if="isMounted">{{ formatDate(session.created_at) }}</template>
                            <template v-else>Loading...</template>
                         </span>
                     </div>
                     <p class="text-[11px] text-foreground/50 line-clamp-1 italic-none truncate w-full flex items-center gap-2">
                        <span class="uppercase tracking-[0.1em] text-[8px] opacity-70 bg-background border border-foreground/10 px-1.5 py-0.5 rounded font-black">{{ session.metadata?.type || 'Web' }}</span>
            ...
            <!-- Sidebar Pagination -->
            <ClientOnly>
                <div v-if="totalPages > 1" class="p-4 border-t border-foreground/5 bg-background flex items-center justify-between">
                    <button 
                    @click="currentPage > 1 ? currentPage-- : null"
                    :disabled="currentPage === 1"
                    class="p-2.5 rounded-xl border border-foreground/5 bg-foreground/5 hover:bg-foreground/10 disabled:opacity-20 transition-all"
                    >
                    <ChevronLeft class="w-4 h-4 text-foreground" />
                    </button>
                    <div class="flex flex-col items-center">
                        <span class="text-[9px] font-black text-foreground/50 uppercase tracking-widest mb-1">Grid Page</span>
                        <span class="text-xs font-bold text-foreground tracking-tighter">{{ currentPage }} <span class="text-foreground/50 mx-1">/</span> {{ totalPages }}</span>
                    </div>
                    <button 
        ...
        <div class="hidden md:flex flex-1 flex-col bg-background relative">
            <template v-if="activeSession">
                <!-- Thread Header -->
                <div class="h-[80px] shrink-0 border-b border-foreground/5 flex items-center justify-between px-8 bg-background/80 backdrop-blur-md absolute top-0 left-0 right-0 z-10">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-xl bg-foreground/10 flex items-center justify-center shadow-lg border border-foreground/5">
                            <User class="w-5 h-5 text-foreground/50" />
                        </div>
                        <div>
                            <h3 class="font-bold text-foreground uppercase tracking-widest text-sm mb-1">{{ activeSession.metadata?.username || activeSession.metadata?.phone || 'Anonymous Entity' }}</h3>
                ...
                <!-- Messages Area -->
                <div class="flex-1 overflow-y-auto p-8 pt-[120px] pb-32 space-y-6 custom-scrollbar bg-[radial-gradient(ellipse_at_center,rgba(var(--foreground-rgb),0.02)_0%,transparent_100%)]">
                    <div v-if="activeSession.chat_messages && activeSession.chat_messages.length === 0" class="flex flex-col items-center justify-center h-full text-foreground/50 opacity-50">
                    ...
                    <template v-for="msg in activeSession.chat_messages" :key="msg.id">
                        ...
                        <div v-if="msg.role === 'assistant'" class="flex gap-4 max-w-[85%] animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div>
                                <div class="bg-foreground/[0.03] border border-foreground/5 rounded-2xl rounded-tl-sm p-4 text-[13px] text-foreground/80 leading-relaxed shadow-xl">
                                    {{ msg.content }}
                                </div>
                                <span class="text-[9px] text-foreground/50 uppercase tracking-widest font-black mt-2 ml-1 block">{{ formatDate(msg.created_at) }}</span>
                            </div>
                        </div>
                        ...
                        <div v-else class="flex gap-4 max-w-[85%] ml-auto justify-end animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div class="flex flex-col items-end">
                                ...
                                <span class="text-[9px] text-foreground/50 uppercase tracking-widest font-black mt-2 mr-1 block">{{ formatDate(msg.created_at) }}</span>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div v-else class="flex-1 flex flex-col items-center justify-center text-foreground/50">
                <MessageCircle class="w-16 h-16 opacity-10 mb-6" />
                <h3 class="text-sm font-bold uppercase tracking-widest mb-2 text-foreground/50">Awaiting Feed Selection</h3>
