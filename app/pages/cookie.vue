  ...
  <div class="relative min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-6 py-20">
      <div class="space-y-6 mb-32">
        <div v-for="c in cookies" :key="c.category" class="glass-card p-10 border-white/5 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            <div class="flex items-center gap-3 mb-4">
              <div class="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary">
                <Cookie class="w-4 h-4" />
              </div>
              <h3 class="text-xl font-bold text-white tracking-tight">{{ c.category }}</h3>
            </div>
            <p class="text-gray-500 font-medium leading-relaxed max-w-lg">{{ c.desc }}</p>
          </div>
          <div :class="[
            'px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest border',
            c.type === 'Required' ? 'bg-primary/10 border-primary/20 text-primary' : 'bg-white/5 border-white/10 text-gray-400'
          ]">
            {{ c.type }}
          </div>
        </div>
      ...
      <main class="prose prose-invert prose-primary prose-lg">
        <section class="mb-24">
        ...
        <div class="mt-40 p-16 rounded-[48px] bg-white/[0.02] border border-white/5 text-center">
          <h3 class="text-3xl font-bold text-white mb-6 tracking-tight">Questions about your data?</h3>
          <p class="text-gray-400 mb-10 font-medium">Read our full Privacy Policy or contact our legal team.</p>
          <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
            ...
            <a href="mailto:legal@replysuite.app" class="text-white font-bold hover:text-primary transition-colors">
              Contact Legal
            </a>
          </div>
        </div>
