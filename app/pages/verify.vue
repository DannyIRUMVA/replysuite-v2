        ...
        <div class="glass-card p-10 md:p-14 bg-background-card border border-foreground/10 rounded-[2rem] shadow-2xl relative overflow-hidden group">
          ...
          <div class="relative z-10 flex flex-col items-center text-center">
            ...
            <div class="w-full max-w-sm mx-auto space-y-6">
              ...
              <button @click="verifyCode" :disabled="verifying || verificationCode.length < 6" class="w-full btn-gradient !py-4 text-base font-bold disabled:opacity-50 flex items-center justify-center gap-2 rounded-xl transition-transform hover:scale-[1.02] active:scale-[0.98]">
                {{ verifying ? 'Verifying...' : 'Verify account' }}
                <ArrowRight v-if="!verifying" class="w-5 h-5" />
              </button>

              <button @click="resendVerification" :disabled="resendLoading" class="w-full py-4 text-sm font-bold text-foreground/40 hover:text-foreground transition-colors border border-transparent hover:border-foreground/5 rounded-xl bg-transparent hover:bg-foreground/5">
