...
useSeoMeta({
  ...
  description: 'Sign in to manage your chatbot, training, and website or WhatsApp deployment.',
})
...
const handleLogin = async (event?: Event) => {
  ...
  const submittedEmail = String(formData?.get('email') || email.value || '').trim()
      ...
      <div class="relative z-10 space-y-6">
        <div class="space-y-3">
          ...
          <p class="text-foreground/50 text-base max-w-xs font-medium leading-relaxed">
            Manage your chatbot, training, website widget, and WhatsApp setup from one dashboard.
          </p>
        </div>

        <div class="grid gap-3 pt-5">
          <div v-for="feat in loginFeatures" :key="feat.title" class="flex items-center gap-3 rounded-[0.39rem] border border-foreground/5 bg-background/35 p-3">
    ...
    <div class="flex-1 flex flex-col justify-center items-center p-5 md:p-9 lg:p-14 relative bg-background">
      ...
      <div class="w-full max-w-[25rem] animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div class="mb-7 text-center lg:text-left">
          <h1 class="text-3xl font-bold tracking-tight mb-2 text-foreground">Sign in</h1>
          <p class="text-sm text-foreground/50 font-medium leading-relaxed">Open your dashboard and continue setup.</p>
        </div>

        <div v-if="verified" class="mb-6 p-3 rounded-[0.39rem] bg-primary/10 border border-primary/20 flex items-center gap-3 animate-in fade-in zoom-in duration-500">
          <div class="w-9 h-9 rounded-[0.39rem] bg-primary/20 flex items-center justify-center text-primary">
        ...
        <div>
          <button @click="signInWithGoogle" :disabled="loading" class="w-full flex items-center justify-center gap-3 py-3.5 rounded-[0.39rem] bg-foreground text-background text-sm font-bold hover:opacity-90 transition-all mb-7 shadow-lg active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45">
          ...
          <form @submit.prevent="handleLogin" class="space-y-6">
            ...
            <div class="flex flex-col gap-3 pt-2">
              <button type="submit" :disabled="loading" class="w-full h-12 bg-gradient-to-tr from-primary to-primary-accent text-black text-sm font-bold rounded-[0.39rem] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-primary/10 disabled:cursor-not-allowed disabled:opacity-50 flex items-center justify-center gap-2">
                <Loader2 v-if="loading" class="w-5 h-5 animate-spin" />
                <span v-else>Sign in</span>
              </button>

              <NuxtLink to="/register" class="w-full py-3.5 text-center text-xs font-bold text-foreground/50 hover:text-foreground transition-colors border border-foreground/5 rounded-[0.39rem]">
                Need an account? <span class="text-primary font-bold">Start free</span>
              </NuxtLink>
            </div>
          </form>
        </div>
