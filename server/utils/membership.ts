...
export const isPaidPlanSlug = (slug?: string | null) => {
  const normalized = normalizePlanSlug(slug);
  return ["silver", "gold", "enterprise-ready", "enterprise"].includes(
    normalized,
  );
};
...
export const isMembershipCurrentlyUsable = (
  membership: any,
  now = Date.now(),
) => {
...
export const selectBestMembership = (
  memberships: any[] = [],
  now = Date.now(),
) =>
  memberships
    ...
    .sort((a, b) => {
      const priorityDiff =
        getPlanPriority(b?.plans?.internal_slug) -
        getPlanPriority(a?.plans?.internal_slug);
...
export const selectBestPlanSlug = (
  memberships: any[] = [],
  fallback = "starter",
) => {
...
export const deactivateOtherActiveMemberships = async (
  supabase: any,
  userId: string,
  keepMembershipId?: string | null,
) => {
  if (!supabase || !userId) return { changed: 0, error: null };
  ...
  if (keepMembershipId) {
    query = query.neq("id", keepMembershipId);
  }

  const { data, error } = await query.select("id");
  return { changed: data?.length || 0, error };
};
