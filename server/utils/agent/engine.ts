...
export const runAgentCycle = async (messages: any[], options: AgentOptions) => {
  ...
  const ownerPlanSlug = await getChatbotOwnerPlanSlug(options.event, options.chatbotId)
  const planAllowedTools = filterAgentToolsForPlan(options.enabledTools, ownerPlanSlug)

  const expandedEnabledTools = planAllowedTools
    ? planAllowedTools.flatMap((feature) => FEATURE_TOOL_MAP[feature] || [feature])
    : []

  const activeTools = TOOLS.map((toolGroup) => ({
    function_declarations: toolGroup.function_declarations.filter((fd) => expandedEnabledTools.includes(fd.name))
