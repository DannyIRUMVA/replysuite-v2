...
const normalizePhoneForPaypack = (phone: string) => sanitizeString(phone, 32).replace(/\s+/g, '').replace(/^\+250/, '0').replace(/^250/, '0')
...
export default defineEventHandler(async (event) => {
  ...
  if (!Number.isFinite(amount) || amount < 100 || amount > 5_000_000) {
    throw createError({ statusCode: 400, statusMessage: 'Amount must be between 100 and 5,000,000 RWF.' })
  }

  const phone = normalizePhoneForPaypack(body?.phone)
  if (!isValidRwandaPhone(phone)) {
    throw createError({ statusCode: 400, statusMessage: 'A valid Rwanda phone number is required.' })
  }

  const transactionRef = sanitizeString(body?.transactionRef || body?.transaction_ref, 120)
  if (!/^[a-zA-Z0-9_-]{8,120}$/.test(transactionRef)) {
    throw createError({ statusCode: 400, statusMessage: 'A valid transaction reference is required.' })
  }

  const projectRef = sanitizeString(body?.project_ref || body?.projectRef, 120)
  const businessName = sanitizeString(body?.business_name || body?.businessName, 160)
  ...
  if (!paymentConfig?.enabled || !paymentConfig?.base_url) {
    throw createError({ statusCode: 409, statusMessage: 'Website payment is not configured for this assistant.' })
  }

  const baseUrl = String(paymentConfig.base_url).replace(/\/+$/, '')
  ...
  await admin.from('chatbot_tool_events').insert({
    ...
    error_message: response.ok ? null : 'Website payment worker rejected the payment request.',
  })
