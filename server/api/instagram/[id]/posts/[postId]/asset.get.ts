...
const svgFallback = (label = 'POST') => {
  const cleanLabel = String(label || 'POST').replace(/[^a-z0-9]/gi, '').slice(0, 6).toUpperCase() || 'POST'
...
export default defineEventHandler(async (event) => {
  ...
  if (!postId) throw createError({ statusCode: 400, statusMessage: 'Invalid Instagram post ID.' })
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Instagram asset returned ${response.status}`)
    ...
    console.warn('[Instagram Asset] Falling back to generated post asset.', error)
    setHeader(event, 'content-type', 'image/svg+xml; charset=utf-8')
    setHeader(event, 'cache-control', 'public, max-age=300')
    return fallback
  }
