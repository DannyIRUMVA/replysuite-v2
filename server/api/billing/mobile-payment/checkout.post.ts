...
const sanitizeString = (value: unknown, maxLength = 500) =>
  String(value || "")
    .trim()
    .slice(0, maxLength);
const normalizePhoneForMobilePayment = (phone: string) =>
...
const SUBSCRIPTION_MOBILE_PLANS: Record<
  string,
  { name: string; amount: number; currency: "RWF" }
> = {
...
const makeTransactionRef = (planId: string, userId: string) => {
  const userPart =
    String(userId || "user")
      .replace(/[^a-zA-Z0-9]/g, "")
      .slice(0, 12) || "user";
  return `replysuite-sub-${planId}-${userPart}-${Date.now()}`
...
export default defineEventHandler(async (event) => {
  ...
  if (!plan) {
    throw createError({
      statusCode: 400,
      statusMessage: "Choose a valid paid plan.",
    });
  }
  ...
  if (!isValidRwandaPhone(phone)) {
    throw createError({
      statusCode: 400,
      statusMessage: "Enter a valid MTN or Airtel Rwanda phone number.",
    });
  }
  ...
  if (phone === TEST_COMPLETED_PAYMENT_PHONE) {
    ...
    if (planError || !dbPlan?.id) {
      console.warn(
        "[Subscription Mobile Payment Test] Plan lookup failed:",
        planError,
      );
      throw createError({
        statusCode: 500,
        statusMessage: "Could not activate the selected test plan.",
      });
    }
    ...
    if (membershipResult.error || !membershipResult.data?.id) {
      console.warn(
        "[Subscription Mobile Payment Test] Membership activation failed:",
        membershipResult.error,
      );
      throw createError({
        statusCode: 500,
        statusMessage: "Could not complete the test payment.",
      });
    }
    ...
    if (deactivateError) {
      console.warn(
        "[Subscription Mobile Payment Test] Membership cleanup failed:",
        deactivateError,
      );
    }
    ...
    if (paymentError) {
      console.warn(
        "[Subscription Mobile Payment Test] Completed payment record insert failed:",
        paymentError,
      );
    }
  ...
  if (!baseUrl) {
    throw createError({
      statusCode: 503,
      statusMessage:
        "MTN/Airtel mobile payment is not configured for subscriptions yet.",
    });
  }
  ...
  if (planError || !dbPlan?.id) {
    console.warn(
      "[Subscription Mobile Payment] Plan lookup failed:",
      planError,
    );
    throw createError({
      statusCode: 500,
      statusMessage: "Could not prepare the selected plan.",
    });
  }
  ...
  if (pendingPaymentError) {
    console.warn(
      "[Subscription Mobile Payment] Pending payment record insert failed:",
      pendingPaymentError,
    );
  }
  ...
  if (!response.ok) {
    console.warn(
      "[Subscription Mobile Payment] Worker rejected payment request:",
      response.status,
      providerResponse,
    );
    await adminClient
    ...
    throw createError({
      statusCode: 502,
      statusMessage:
        "MTN/Airtel mobile payment request failed. Please try again or use card payment.",
  ...
  return {
    ...
    transactionRef,
    message:
      "Payment prompt sent. Complete it on your phone, then contact support or use plan sync after confirmation.",
    providerResponse,
  };
});
