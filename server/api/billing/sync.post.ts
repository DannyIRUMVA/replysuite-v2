...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log(
      `[Polar Sync] Manual sync triggered for ${userId} (${userEmail})`,
    );
    ...
    try {
      ...
      if (customer) {
        ...
        foundSubscription = subs.result?.items?.find((s) =>
    ...
    if (!foundSubscription) {
      return {
        success: true,
        message:
          "No active Polar subscription found. Internal plan remains unchanged.",
      };
    }
    ...
    if (plan) {
      ...
      if (deactivateError)
        console.warn(
          "[Polar Sync] Membership cleanup failed:",
          deactivateError,
        );

      return {
        success: true,
        message: `Synced! You are currently on the ${plan.name} plan.`,
      };
    }
    ...
    return {
      success: false,
      message:
        "Polar subscription found but no matching local plan configuration exists.",
    };
  } catch (err: any) {
    console.error("[Polar Sync Error]", err.message);
    throw createError({
      statusCode: 500,
      message: "Failed to sync. Please try again later.",
    });
  }
});
