...
export default defineEventHandler(async (event) => {
  try {
    ...
    let polarCustomerId: string | undefined =
      localMembership?.polar_customer_id;

    // ── Step 1: Exhaustive Active Subscription Detection ──
    console.log(
      `[Polar Upgrade] Starting exhaustive sub search for user: ${userId} (${userEmail})`,
    );
    ...
    if (localMembership?.polar_subscription_id) {
      try {
        console.log(
          `[Polar Upgrade] Found local sub ID: ${localMembership.polar_subscription_id}. Verifying with Polar...`,
        );
        const sub = await polar.subscriptions.get(
          localMembership.polar_subscription_id,
        );
    ...
    if (!activeSubscription) {
      try {
        ...
        if (customer) {
          ...
          activeSubscription = subs.result?.items?.find((s) =>
            ["active", "trialing", "past_due"].includes(s.status),
          );
          if (activeSubscription) {
            console.log(
              `[Polar Upgrade] Found active sub via customer lookup: ${activeSubscription.id}`,
            );
          }
        }
      } catch (e) {
      ...
      if (!activeSubscription && userEmail) {
        try {
          ...
          for (const customer of customerItems) {
            ...
            const activeSub = subs.result?.items?.find(
              (s) => s.status !== "canceled",
            );
            ...
            if (activeSub) {
              ...
              polarCustomerId = customer.id;
              console.log(
                `[Polar Upgrade] Found active sub via email search: ${activeSubscription.id} (Customer: ${polarCustomerId})`,
              );
              break;
            }
          }
    ...
    if (activeSubscription && activeSubscription.productId === productId) {
      console.log(
        `[Polar Upgrade] User already has product ${productId}. Skipping...`,
      );
      return {
        upgraded: true,
    ...
    if (!polarCustomerId) {
      if (!userEmail) {
        throw createError({
          statusCode: 400,
          message: "Authenticated email is required for billing.",
        });
      }
      console.log(
        `[Polar Upgrade] No customer ID found. Creating/linking stable Polar customer.`,
      );
      ...
      if (!polarCustomerId) {
        throw createError({
          statusCode: 500,
          message: "Could not establish Polar customer.",
        });
      }
    }
    ...
    if (activeSubscription) {
      ...
      if (activeSubscription.productId === productId) {
        console.log(
          `[Polar Upgrade] User already has this product on Polar. Syncing local DB and returning early.`,
        );
      ...
      console.log(
        `[Polar Upgrade] Generating upgrade checkout for: ${activeSubscription.id} -> ${productId}`,
      );
      try {
        ...
        console.error(
          "[Polar Upgrade] Upgrade checkout failed:",
          updateErr.message,
        );
    ...
    console.log(
      `[Polar Upgrade] No active sub found. Diverting to checkout for: ${productId}`,
    );
    ...
    try {
      ...
      if (isAlreadyActive) {
        console.log(
          `[Polar Upgrade] Polar reported AlreadyActiveSubscriptionError. performing emergency sync for ${userEmail}...`,
        );
        ...
        for (const customer of customers.result?.items || []) {
          ...
          const active = subs.result?.items?.find(
            (s) => s.status !== "canceled",
          );

          if (active) {
            console.log(
              `[Polar Upgrade] Found conflicting sub ${active.id} for product ${active.productId}`,
            );
            await syncLocalMembership(
              adminClient,
              userId,
            ...
            return {
              upgraded: true,
              message:
                "You already have an active subscription! We have synced your records.",
 ...
 */
async function syncLocalMembership(
  supabase: any,
  userId: string,
  productId: string,
  subId?: string,
  ...
  try {
    ...
    if (plan) {
      ...
      if (deactivateError)
        console.warn(
          "[Polar Upgrade] Membership cleanup failed:",
          deactivateError,
        );
      console.log(
        `[Polar Upgrade] Local membership synced for plan: ${plan.id}`,
      );
    }
  } catch (e) {
