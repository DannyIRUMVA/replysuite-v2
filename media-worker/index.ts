...
const allowedMimeTypes = (env: Env) =>
  new Set(
...
const buildAssetUrl = (request: Request, env: Env, key: string) => {
  const base = String(env.MEDIA_PUBLIC_BASE_URL || "")
    .trim()
    .replace(/\/+$/, "");
...
const upload = async (request: Request, env: Env): Promise<Response> => {
  ...
  if (!contentType.includes("multipart/form-data")) {
    return json(
      { success: false, error: "Use multipart/form-data with a file field." },
      { status: 415 },
    );
  }
  ...
  const chatbotSlug = sanitizeSegment(
    form.get("chatbot") || form.get("chatbot_slug"),
    "chatbot",
  );
  const sessionSlug = sanitizeSegment(
    form.get("session") || form.get("session_slug"),
    "session",
  );
  const label = sanitizeSegment(
    form.get("label") || file.name || "image",
    "image",
  );
...
const publicAssetPrefixes = (env: Env) =>
  String(env.PUBLIC_ASSET_PREFIXES || "whatsapp/")
    .split(",")
    .map((item) => item.trim().replace(/^\/+/, ""))
    .filter(Boolean);

const isPublicAsset = (env: Env, key: string) =>
  publicAssetPrefixes(env).some((prefix) => key.startsWith(prefix));
...
const serveAsset = async (
  ...
  if (!isPublicAsset(env, safeKey) && !requireBearer(request, env)) {
    return new Response("Not found", { status: 404 });
  }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      return json({
        ...
        publicAssetPrefixes: publicAssetPrefixes(env),
      });
    }
